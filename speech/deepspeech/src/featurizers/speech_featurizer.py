...
class SpeechFeaturizer(tf.keras.layers.Layer):
    ...
    def preemphasis_signal(self, signal):
        if not self.preemphasis or self.preemphasis <= 0.0:
            return signal
        # print("EXPAND DIMS",signal[0].shape)
    ...
    def call(self, inputs, training=False):
        ...
        compute_method = feature_compute_methods.get(self.feature_type, None)
        ...
        print("signals length:", signals_length)
        # features_length = tf.map_fn(
        #     fn=self.get_nframes,
        #     elems=tf.cast((signals_length), tf.int32),
        #     fn_output_signature=tf.TensorSpec(shape=(), dtype=tf.int32),
        # )
        features_length = self.get_nframes(tf.cast(signals_length, tf.int32))

        if training:
            features, features_length = self.augmentation.feature_augment(features, features_length)
