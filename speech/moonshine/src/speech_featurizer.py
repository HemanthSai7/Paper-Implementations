...
class SpeechFeaturizer(tf.keras.layers.Layer):
    ...
    def compute_output_shape(self, input_shape):
        ...
        audio_shape = input_shape[0]
        batch_dim = audio_shape[0]
        seq_dim = audio_shape[1]

        if seq_dim is not None and self.padding is not None:
            output_seq_dim = seq_dim + self.padding
        else:
            output_seq_dim = None
        
        return (batch_dim, output_seq_dim, 1)
